...
for way in  zeropad_pre zeropad_outer zeropad_mid zeropad_post noisepad_pre \
noisepad_outer noisepad_post edgepad_pre edgepad_outer edgepad_post \
interpolate strf_pad random_pad zoom_pad truncate_pre truncate_outer truncate_post
do
for file in AsphaltPavementType AsphaltRegularity AsphaltObstacles PLAID \
AllGestureWiimoteX AllGestureWiimoteY AllGestureWiimoteZ GestureMidAirD1 GestureMidAirD2 \
GestureMidAirD3 GesturePebbleZ1 GesturePebbleZ2 PickupGestureWiimoteZ  ShakeGestureWiimoteZ
do
mode=${way_mode[$way]}
python -u ../run.py \
--is_training 1 \
